...
def build_train_neural_linear_old(make_obs_ph, q_func, num_actions, optimizer, grad_norm_clipping=None, gamma=1.0,
                                  double_q=True, scope="deepq", reuse=None, param_noise=False,
                                  param_noise_filter_func=None,
                                  embedding_network=None, contingency_network=None, local_scale=50):

    act_f = build_act_thompson(make_obs_ph, q_func, num_actions, scope=scope, reuse=reuse, method="neural linear")
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        # compute estimate of best possible value starting from state at t + 1 - DDQN

        # DDQN when actor is target + thompson sampling
        if False:  # not sure what ddqn means when actor held this way TODO
            phi_target_xtp1 = tf.expand_dims(phi_target_xtp1, axis=-1)
            q_tp1_using_online_net = tf.squeeze(tf.matmul(w_ph, phi_target_xtp1))
        ...
        if grad_norm_clipping is not None:
            ...
            optimize_expr = optimizer.minimize(weighted_error, var_list=trainable_vars)
